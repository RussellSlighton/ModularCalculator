...
this.run = function() {
    ...
    e = new Environment([],[],culled,this.nodeDict.reverse());
  var ia = e.run()
    return ia;
  }
...
this.parseText = function(expression) {
  ...
  var them = []
  them = this.identifyExpressions(expression);

  var expressions =them[0];

  var locations = them[1];
  
  var func = this.identifyFunction(expression, expressions,locations);
...
this.identifyExpressions = function(nakedExpression) {
  ...
  for (var x = 0; x < nakedExpression.length; x++) {
    ...
    if (openIndicies.length == closeIndicies.length) {
      var ting = nakedExpression.substring(openIndicies[0] + 1, closeIndicies[closeIndicies.length - 1]);
      if(ting != nakedExpression){
        expressions.push(ting);
        locations.push(openIndicies[0]+1);
      }
      openIndicies = [];
      closeIndicies = [];
    }
  ...
  return [expressions, locations];
};

this.identifyFunction = function(nakedExpression, subExpressions, expressionLocations) {
...
;
  var syntaxPattern = this.getSyntaxPattern(nakedExpression, subExpressions, func, expressionLocations);
...
this.getSyntaxPattern = function(nakedExpression, subExpressions, func, expressionLocations) {
